...
<p style="margin:0;font-size:12px;color:#555;">Size: ${item.size || "-"}</p>
        ...
        return `
                <tr style="border-bottom: 1px solid #eee;">
                   ...
                   <td style="padding: 10px;">
                        ...
                        ${order.items.map(i =>
                          `${i.name} (Size ${i.size || "-"}) x${i.quantity}`
                        ).join("<br>")}
                        </div>
                    </td>
  ...
  if (details) {
    details.innerHTML = `
            <div style="background: #fff; padding: 20px; border: 2px dashed #000; font-family: monospace;">
                ...
                <hr>
               ${order.items.map((i) => `<div style="display:flex; justify-content:space-between; font-size:12px;"><span>${i.name} (Size ${i.size || "-"}) x${i.quantity}</span><span>${(i.price * i.quantity).toLocaleString()}₫</span></div>`).join("")}
                <hr>
  ...
  productGrid.innerHTML = filteredProducts
    .map(
      (product) => `
          ...
          <a href="productss.html?id=${product.id}" style="text-decoration:none;color:inherit">

              <div class="product-img">
                  <img src="${product.image}" alt="${product.name}">
                  <button class="quick-add" onclick="addToCart(${product.id});event.preventDefault();">
                      THÊM VÀO GIỎ +
                  </button>
              </div>
