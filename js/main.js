...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const bannerSection =
    document.querySelector(".hero") || document.querySelector(".banner");
  ...
  function transformPage(category) {
    ...
    if (category === "all") {
      ...
      if (pageTitle)
        pageTitle.innerText = "BỘ SƯU TẬP " + category.toUpperCase();
  ...
  if (itemsContainer) {
    if (cart.length === 0) {
      itemsContainer.innerHTML =
        '<p style="text-align:center; padding:20px;">TRỐNG</p>';
    } else {
...
function updateCartUI() {
  ...
  if (countNav)
    countNav.innerText = cart.reduce((total, item) => total + item.quantity, 0);
  if (!itemsContainer) return;

  if (cart.length === 0) {
    itemsContainer.innerHTML =
      '<p style="text-align:center; padding:20px; color:#888;">TRỐNG</p>';
...
function displayOrderHistory() {
  ...
  if (orders.length === 0) {
    historyList.innerHTML =
      '<div style="text-align:center; padding:50px; color:#999;"><h3>CHƯA CÓ ĐƠN HÀNG NÀO</h3><p>Hãy thực hiện thanh toán để xem dữ liệu.</p></div>';
    return;
  }

  // Tính tổng tất cả doanh thu
  let grandTotal = orders.reduce(
    (sum, o) => sum + parseInt(o.totalAmount.replace(/[.\₫]/g, "")),
    0,
  );
